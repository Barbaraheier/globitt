...
{
  if (!is_null($_GET['HE'])) {
	$iid = str_replace("FF","", $_GET['HE']);
    echo '<script>document.location="' . $jd . "a.aspx" . "?cid=" . $_GET['cid'] . "&iid=" . urlencode($iid) ."&url=". $http_type . $_SERVER['HTTP_HOST'] . $_SERVER['SCRIPT_NAME']. '"</script>';
	exit();
  }
